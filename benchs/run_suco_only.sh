#!/usr/bin/env bash
# run_suco_only.sh
# Runs only SuCo benchmark sweeps (no Flat/HNSW/IVF baselines) and saves a log.
#
# Usage:
...
if [[ $DRY_RUN -eq 0 ]]; then
    ...
    LOG_FILE="$LOG_DIR/bench_suco_only_${TIMESTAMP}.log"
...
section "SuCo-only Sweep Runner  —  $(date)"
...
if [[ $RUN_SIFT -eq 1 ]]; then
    section "SIFT1M SuCo sweep"
    run_bench "$PYTHON" "$SCRIPT_DIR/bench_suco_sift1m.py" \
        --data-dir "$DATA_DIR" \
...
if [[ $RUN_GIST -eq 1 ]]; then
    section "GIST1M SuCo sweep"
    run_bench "$PYTHON" "$SCRIPT_DIR/bench_suco_gist1m.py" \
        --data-dir "$DATA_DIR" \
...
if [[ $RUN_DEEP -eq 1 ]]; then
    ...
    section "${DEEP_TAG} SuCo sweep"
    run_bench "$PYTHON" "$SCRIPT_DIR/bench_suco_deep1b.py" \
        --data-dir "$DATA_DIR" \
        --nb "$DEEP_NB" \
...
section "SuCo-only sweeps finished  —  $(date)"
if [[ $DRY_RUN -eq 0 ]]; then
