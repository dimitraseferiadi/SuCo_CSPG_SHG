#!/usr/bin/env python3
"""
benchs/bench_suco_sift10m.py

Benchmark IndexSuCo (and HNSW / IVFFlat / IVFPQ / OPQ+IVFPQ baselines)
on the SIFT10M dataset  (d=128, uint8 SIFT features).
...
Notes
-----
*  Vectors are stored as uint8 and cast to float32 before indexing.
   uint8 coords are in [0, 255]; the cast is lossless for L2 search.

*  Valid nsubspaces for d=128 (subspace_dim = d/Ns must be even):
   1, 2, 4, 8, 16, 32, 64  →  default: 8  (8 subspaces × 16 dims each)

*  Valid PQ m values for d=128 (m must divide d):
   2, 4, 8, 16, 32, 64, 128  →  default: 16

*  Logs are written to both stdout and
       logs/bench_suco_sift10m_YYYYMMDD_HHMMSS.log
   (relative to the repository root, i.e. the directory two levels above
   this file).
"""
...
def compute_gt(xb: np.ndarray, xq: np.ndarray, k: int = 100,
    ...
    Batches the search to keep peak memory bounded.
    """
    d  = xb.shape[1]
    nq = xq.shape[0]
...
# d=128: valid nsubspaces (subspace_dim = d/Ns must be even): 1,2,4,8,16,32,64
SWEEP_CONFIGS = [
    # (nsubspaces, ncentroids_half, collision_ratio, candidate_ratio)
    # ---- varying collision_ratio (Ns=8 default) ----
    (8, 50, 0.02, 0.002),
    (8, 50, 0.05, 0.005),   # default
    (8, 50, 0.10, 0.010),
    (8, 50, 0.20, 0.020),
    # ---- varying nsubspaces ----
    (4,  50, 0.05, 0.005),
...
LSH_NBITS_SWEEP = [128, 256, 512, 1024]
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Benchmark IndexSuCo on SIFT10M (d=128, uint8 → float32).",
    ...
    p.add_argument(
        ...
        help="Number of training vectors for SuCo (randomly sampled from xb).",
    )
    p.add_argument(
...
def main():
    ...
    # ------------------------------------------------------------------
    if args.lsh:
        print_header("IndexLSH benchmark (SIFT10M, d=128)") 
