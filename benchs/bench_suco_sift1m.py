#!/usr/bin/env python3
"""
benchs/bench_suco_sift1m.py

Benchmark faiss.IndexSuCo (and optionally IndexHNSWFlat / IndexFlatL2) on
the SIFT1M dataset  (d=128, nb=1 000 000, nq=10 000).

Usage
-----
# Default run (nsubspaces=8, half_dim=8):
    python benchs/bench_suco_sift1m.py --data-dir /path/to/data/

# Parameter sweep over nsubspaces and collision_ratio:
    python benchs/bench_suco_sift1m.py --data-dir /path/to/data/ --sweep

# HNSW efSearch sweep for recall/QPS trade-off:
    python benchs/bench_suco_sift1m.py --data-dir /path/to/data/ --hnsw-sweep

# Flat brute-force baseline (upper bound):
    python benchs/bench_suco_sift1m.py --data-dir /path/to/data/ --flat-baseline

# Save/reload index to skip rebuild on repeated runs:
    python benchs/bench_suco_sift1m.py --data-dir /path/to/data/ --index-path /tmp/suco_sift.idx

Dataset preparation
-------------------
Download from http://corpus-texmex.irisa.fr/ (ANN_SIFT1M, ~161 MB):

    mkdir -p /path/to/data/sift1M && cd /path/to/data/sift1M
    wget http://corpus-texmex.irisa.fr/ANN_SIFT1M.tar.gz
    tar -xzf ANN_SIFT1M.tar.gz
...
Valid choices for d=128 and their half-subspace dimensions:

    nsubspaces   half_dim   comment
    ----------   --------   -------
         4          16
         8           8      ← default; matches paper's Deep1B ratio
        16           4
        32           2      very small halves — may hurt quality
"""
...
# All entries must satisfy: 128 % nsubspaces == 0 and (128 // nsubspaces) % 2 == 0
SWEEP_CONFIGS = [
    # (nsubspaces, ncentroids_half, collision_ratio, candidate_ratio)
    # ---- varying nsubspaces (half_dim shown in comment) ----
    ( 4, 50, 0.05, 0.005),   # half_dim=16
    ( 8, 50, 0.05, 0.005),   # half_dim=8   ← paper-equivalent default
    (16, 50, 0.05, 0.005),   # half_dim=4
    # ---- varying collision_ratio at default nsubspaces=8 ----
    ( 8, 50, 0.02, 0.002),
    ( 8, 50, 0.10, 0.010),
    ( 8, 50, 0.20, 0.020),
    # ---- varying ncentroids_half at default nsubspaces=8 ----
    ( 8, 25, 0.05, 0.005),
    ( 8, 100, 0.05, 0.005),
]
...
def run_sweep(xb, xq, xt, gt, k: int = 10) -> None:
    print_header("Parameter Sweep  (SIFT1M, d=128)")
    header = (
...
def compare_vs_flat(xb: np.ndarray, xq: np.ndarray, gt: np.ndarray,
                    k: int = 10) -> None:
    print_header("Baseline: IndexFlatL2  (SIFT1M)")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Benchmark IndexSuCo on SIFT1M (d=128, nb=1M, nq=10000).",
    ...
    p.add_argument(
        ...
        help="Number of training vectors (100K available in sift_learn.fvecs).",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Valid choices: 1, 2, 4, 8, 16, 32, 64, 128."
        ),
    )
    p.add_argument(
...
def main():
    ...
    print_header("IndexSuCo benchmark  (SIFT1M, d=128)")
    ...
    # ------------------------------------------------------------------
    if args.hnsw:
        print_header("IndexHNSWFlat benchmark  (SIFT1M, d=128)")
    ...
    # ------------------------------------------------------------------
    if args.ivfflat:
        print_header("IndexIVFFlat benchmark  (SIFT1M, d=128)")
    ...
    # ------------------------------------------------------------------
    if args.ivfpq:
        print_header("IndexIVFPQ benchmark  (SIFT1M, d=128)")
        print(f"  nlist           = {args.nlist}")
        print(f"  nprobe          = {args.nprobe}")
    ...
    # ------------------------------------------------------------------
    if args.opqpq:
        print_header("OPQ+IVFPQ benchmark  (SIFT1M, d=128)")
        print(f"  nlist           = {args.nlist}")
        print(f"  nprobe          = {args.nprobe}")
        print(f"  pq_m            = {args.pq_m}")
        print(f"  pq_nbits        = {args.pq_nbits}")
